...
ex.add_config("configs/iv_scp.yaml")
...
class Objective:
  ...
  def __init__(self, dim_x, psi_vector, store_data, sample_size=500):
  ...
  def __call__(self, model, xstar):
    mu, _ = model(self.distr.sample(torch.Size([self.sample_size])))
    psi_xstar = self.psi_vector(xstar)
    exp_theta = torch.mean(mu, axis=0)
    return torch.dot(exp_theta, psi_xstar)
...
class Constraints:
  ...
  def __init__(self, lhs, n_inferred, psi_vector, dim_theta, constr_indices,\
               constr_type, slack, save_rhs_every_step, store_data):
  ...
  def __call__(self, model, results, opt_stp, data):
    ...
    mu_theta, vec_theta = model(self.n_inferred[self.constr_indices])
    sigma_theta = utils.vec_to_sigma(vec_theta, self.dim_theta)

    psi_x = self.psi_vector(data['x'][self.constr_indices])
...
class AugmentedLagrangian:
  ...
  def optimize(self,
    ...
    model = utils.Gaussian(self.dim_x, _config['dim_theta']) 
    model = utils.initialize_model_weights(self.data['x'], model, self.psi_vector, self.lhs_all,\
                                           self.n_inferred, _config['dim_theta'])
    ...
    for rnd in range(n_rounds):
      ...
      for opt_stp in range(opt_steps):
        ...
        obj = self.objective(model, xstar)
...
def main(_config, _log):
  ...
  if(_config['generate_data'] == True):
    dat = data.get_data('IV', _config['dataset'], _config['equations'], _config['num_data'],\
  ...
  if(_config['response_type'] == 'mlp'):
    ...
    psi_vector = utils.psi_polyn(dim_x, _config['dim_theta'])
  _log.info("Get inferred N...")
  n_inferred = utils.get_n_inferred(dim_z, dim_x, dat['z'], dat['x'], _config['model_xz'])
  indices_to_remove = utils.low_likelihood_indices(n_inferred, dim_x, _config['num_to_remove'])
  ...
  constr_indices = utils.get_indices(dat, _config['num_constant_samples'], _config['seed_data'])
  ...
  lhs, lhs_all = utils.get_lhs(dim_x + dim_z, zx, dat['y'], constr_indices)  
  
  # Initialize the objective, constraint and augmented lagrangian classes
  objective = Objective(dim_x, psi_vector, _config['store_data'])
